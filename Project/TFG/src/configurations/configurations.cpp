...
static void long_pulse_action(Buttons::BUTTON_SYMBOL previous_symbol, uint32_t us_cycle, std::function<void ()> action);
/* Functions -----------------------------------------------------------------*/
    ...
    while(true){
      ...
      if(pulsed.symbol == Buttons::UP || pulsed.symbol == Buttons::DOWN){
        ...
        if(pulsed.type == Buttons::SHORT_PULSE)
          ...
          long_pulse_action(pulsed.symbol, 100000, action);
      }
    }
  }
  return -1;
 ...
 * @return 0 if success, -1 if error.
 */
int Configurations::handle_server_mqtt_token(std::string *mqtt_token){

  uint8_t min_char = 0x20, max_char = 0x7F;
  uint32_t pos = 0, pos_range = mqtt_token->size();
  ...
  auto action_builder = [&]() -> std::function<void ()>{
    if(pulsed.symbol == Buttons::UP)
      return [&]() {
        if(++(*mqtt_token)[pos] == max_char)
          (*mqtt_token)[pos] = (pos == mqtt_token->size() - 1) ? (min_char) : (min_char + 1);
        std::cout << *mqtt_token << " " << mqtt_token->size() << std::endl;
      };
    else // if(pulsed.symbol == Buttons::DOWN)
      return [&]() {
        if(pos == mqtt_token->size() - 1){
          if(--(*mqtt_token)[pos] == min_char - 1)
            (*mqtt_token)[pos] = max_char - 1;
        }
        else{
          if(--(*mqtt_token)[pos] == min_char)
            (*mqtt_token)[pos] = max_char - 1;
        }
        std::cout << *mqtt_token << " " << mqtt_token->size() << std::endl;
      };
  ...
  while(true){
    ...
    if(pulsed.symbol == Buttons::LEFT){
      pos = (pos + pos_range - 1) % pos_range;

      if(pos == pos_range - 2 && mqtt_token->back() == min_char){
        pos_range -= 1;
        mqtt_token->pop_back();
      }
      ...
      if(pos == pos_range - 1){
        if(mqtt_token->back() != min_char){
          pos++;
          pos_range++;
          mqtt_token->push_back(min_char);
        }
      }
      else{
        pos++;
    ...
    if(pulsed.symbol == Buttons::UP || pulsed.symbol == Buttons::DOWN){
      ...
      if(pulsed.type == Buttons::SHORT_PULSE)
        ...
        long_pulse_action(pulsed.symbol, 50000, action);
    }
  }
  return -1;
}
...
int Configurations::handle_temp_offset(float *offset){
  ...
  offset_int = int(std::ceil((float)(*offset * 100.0))) % 1000; //Integer version to work only with integers, so the calculus are
...
static void long_pulse_action(Buttons::BUTTON_SYMBOL previous_symbol, uint32_t us_cycle, std::function<void ()> action){
  ...
  do{
    ...
    if(Buttons::buttonsQueue.size() > 0){
      ...
      pressed = aux_pulsed.symbol != previous_symbol || aux_pulsed.type != Buttons::LONG_PULSE_RELEASED;
    }
  }while(pressed);
}
