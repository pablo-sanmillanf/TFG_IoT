...
void Display_driver::write_fast_string(int start_x, int start_y, char str[], uint8_t color[], uint8_t background_color[], GFXfont font){
...
void Display_driver::write_fast_string_centered(int start_y, char str[], uint8_t color[], uint8_t background_color[], GFXfont font){
...
void Display_driver::write_fast_string_truncated(int start_x, int start_y, int truncated_width, char str[],
    uint8_t color[], uint8_t background_color[], GFXfont font){

  int y_offset, width;
  std::vector<uint16_t> string_bytes = prepare_string(str, color, background_color, font, &y_offset, &width);

  int height = string_bytes.size()/width;
  uint16_t result[truncated_width*height];

  for(int i = 0; i < height; i++){
    std::copy(
        string_bytes.begin() + i*width,
        string_bytes.begin() + i*width + truncated_width,
...
void Display_driver::write_panning_fast_string(int start_x, int start_y, int panning_width, int panning_step,
